...
export const Minimap: React.FC<MinimapProps> = ({ 
  ...
  return (
    ...
    >
      ...
      >
        {/* Draw edges */}
        {showLines && nodes.filter(n => n.parentId).map(node => {
        ...
        {nodes.map(node => {
          ...
          return (
            <div 
              ...
              className="absolute bg-black rounded-[1px] hover:bg-yellow-400 hover:scale-125 transition-all cursor-pointer pointer-events-auto z-10"
              title={`Zoom to: ${node.prompt}`}
              style={{
                left: nx,
                top: ny,
                width: Math.max(nw, 4),
